# PawSpace - Spec and User Flow Document
...
|---|---|---|---|
| Prospective Owner, breed in mind | Dog or Cat | Yes | A suitability assessment for that specific breed |
| Prospective Owner, no breed in mind | Dog, Cat, or open to either | No | A ranked breed suggestion list |
| Current Owner | Dog or Cat | Yes, their own pet | A suitability assessment, always framed as improvement, never as a reason to give the pet up |
...
| ID | Feature | Task (user) | Action/Output (system) |
|---|---|---|---|
| P3.1 | Breed profile attachment | Enters a known breed, or picks one from a suggested list | Attaches a breed profile (size class, energy level, space orientation, noise tendency) to the user's space |
...
| P3.4 | Not-ready-yet advisory | (Prospective owner only) Sees this as the lead finding when triggered | Avatar stays visibly concerned throughout the walkthrough; report leads with this finding; never appears for Current Owners (see Section 4.6) |
...
Every room the pet has access to gets two internal bands, **Poor / Adequate / Good**, recomputed on every scan and rescan:

- **Space Adequacy**: is there enough of the right kind of space (floor, for dogs; vertical, for cats), weighted by the attached breed's size and energy class. Formula in 4.2 (dogs) and 4.3 (cats).
- **Safety**: hazards, valuables at risk, and whether off-limit zones are actually closable. Formula in 4.4.
...
A third signal, **Noise Fit** (4.5), is computed once for the whole home rather than per room, since it depends on the breed's noise tendency and how close the neighbours are, not on anything room-specific. It doesn't drive the avatar's room-by-room movement; it shows as its own line in the breed detail screen (5.2) and as a top-of-report line in the summary (6.3).
...
### 4.2 Space Adequacy: Dogs (floor-oriented)
...
### 4.3 Space Adequacy: Cats (vertical-oriented)

**Measured parameter:** Vertical Surface Score (VSS) = count of distinct reachable elevated resting/perch points at least 0.8m from the floor in the room (shelving, cat trees, secured furniture tops), plus whether at least one of them is an enclosed/hidden refuge spot rather than an open one.

```
VSS(room) = count(perch_points(room) where height_from_floor >= 0.8m)

space_adequacy(room, cat) =
    Poor       if VSS(room) == 0
    Good       if VSS(room) >= 3 AND has_refuge_spot(room)
    Adequate   otherwise
```

Same thresholds regardless of the cat's size, since vertical need doesn't scale with weight the way floor need does for dogs. Ceiling height isn't scored directly, but a room under roughly 2.1m, or with no furniture near a wall, contributes 0 regardless of floor area.

### 4.4 Safety (both species)
...
| Valuables at risk per room (tagged valuable, below the species' reach height*) | 4+ | 1-3 | 0 |

\* Reach height differs by species: for dogs, items below roughly 1m (paw/mouth reach, counters included); for cats, effectively anything not behind a closed door or on a high closed shelf, since cats can reach most surfaces.
...
### 4.5 Noise Fit (whole-home, not per room)

**Measured/declared parameters:** the attached breed's noise tendency (Quiet / Moderate / Vocal, part of the breed profile, 4.7) and how close the neighbours are (from questionnaire 1.6: Attached walls / Close, separate building / Not close by).
...
### 4.6 "Not ready for a pet yet" advisory: trigger rule
...
Every breed (in the catalogue, or in the suggestion engine) is described by the same schema, so 4.2-4.5 can run against any of them:

| Field | Values | Used by |
|---|---|---|
| Species | Dog / Cat | Determines whether 4.2 (floor) or 4.3 (vertical) applies |
| Size class (dogs only) | Small / Medium / Large, from weight | 4.2 |
| Energy level | Low / Moderate / High | 4.2 (class shift, outdoor-access downgrade) |
| Space orientation | Floor / Vertical | Routing to 4.2 or 4.3 |
| Noise tendency | Quiet / Moderate / Vocal | 4.5 |

Three example profiles, chosen to each exercise different parts of the logic:

| | French Bulldog | Border Collie | Ragdoll (cat) |
|---|---|---|---|
| Species | Dog | Dog | Cat |
| Weight | ~11kg | ~18kg | n/a |
| Size class | Small | Medium | n/a |
| Energy level | Low | High | Low |
| Effective class (4.2) | Small (no shift) | Large (shifted up from Medium) | n/a |
| Space orientation | Floor | Floor | Vertical |
| Noise tendency | Moderate | Vocal | Quiet |

The Border Collie is deliberately picked to be Medium-sized by weight but High-energy, so it's evaluated against the Large-dog thresholds, this is the clearest way to show the class-shift rule actually firing rather than just describing it.

### 4.8 Worked example: French Bulldog

Apartment, attached walls, no outdoor access.

**First scan, living room:** 14m² floor, 7m² furniture footprint, OFR = 50%. Small-class Good threshold is 40%+, so OFR alone clears Good, but no personal-space zone exists yet, so Space Adequacy caps at **Adequate**. 0 hazards, 0 valuables at risk, Safety = **Good**. Room tone = worse_of(Adequate, Good) = **Adequate**. Noise Fit = Moderate breed × Attached walls = **Adequate** (from the 4.5 table).
...
### 4.9 Worked example: Border Collie, two homes
...
### 4.10 Worked example: Ragdoll (cat)

Apartment, attached walls. Shows the full Poor to Good progression for VSS across two rescans, and a Noise Fit that never becomes a problem.

**First scan, bedroom:** 0 elevated perch points, VSS = **0**, **Poor**. Room tone = Poor (Safety is Good, but Space Adequacy drives the room). Avatar: sad, stays low/close to the floor. Speech bubble: *"I need space to jump up."* Noise Fit = Quiet breed × Attached walls = **Good** regardless of the building, it stays Good for the rest of this example no matter what changes spatially.

**User action:** adds a wall shelf and a cat tree with an enclosed pod.

**First rescan:** VSS = 2 (shelf + tree). Even though the tree's pod satisfies the refuge-spot condition, Good requires VSS ≥ 3, so this stays **Adequate**. Avatar: lightly happy, more upright. Speech bubble: *"Getting better, I like that shelf."*

**User action:** adds a wall-mounted step bridging to a windowsill.

**Second rescan:** VSS = 3, refuge spot already present from the tree's pod, both conditions for Good are met. Space Adequacy = **Good**. Avatar: happy, upright and relaxed. Speech bubble: *"I've got the whole room mapped out now, love it up here."*
...
- Subtext: "See how ready your home is for a dog or cat, before or after you bring one home."
...
**1.1 Pet status**
- Question: "Do you already have a pet, or are you thinking about getting one?"
- Options: "I already have a dog" / "I already have a cat" / "I have a breed in mind" / "I'm not sure yet, suggest something"
- Branch: "I already have a dog/cat" leads to Current Owner path. "I have a breed in mind" leads to Prospective (breed in mind). "I'm not sure yet" leads to Prospective (no breed in mind).

**1.2 Species preference** (Prospective, no breed in mind, only)
- Question: "Open to a dog, a cat, or either?"
- Options: "Dog" / "Cat" / "Either, show me both"

**1.3 Breed entry** (Current Owner, and Prospective with a breed in mind)
- Question: "What breed is your dog/cat?" (Current Owner) or "What breed are you considering?" (Prospective)
- Searchable field with autocomplete
- Fallback: "Not sure / mixed breed" leads to "Roughly what size?" (Small, under 12kg / Medium, 12-25kg / Large, over 25kg) and, for dogs, "Coat length?" (Short / Long)

**1.4 Activity level**
- Question: "How much daily activity space would this pet need?"
- Options: "Low, happy to mostly relax" / "Moderate, regular play and movement" / "High, needs to run or climb often"

**1.5 Outdoor access**
- Question: "Does this home have any outdoor space your pet could use?"
...
- Feeds the Noise Fit signal (Section 4.5).

**1.7 Summary and confirm**
...
- Tap any object to mark it valuable or fragile, flagged for the "at risk" logic in Section 4.4.
...
- A ranked list, filtered to the species chosen in 1.2 (or both). Each row: breed name, thumbnail, a one-line qualitative fit summary (e.g. "Good fit, enough floor space, no noise concerns"), no score.
...
- Opens directly into 5.2 for the breed entered in 1.3.
- If the Section 4.6 trigger fires, this is the first place it surfaces: the breed detail screen leads with the advisory line, avatar shown sad, before the per-factor breakdown underneath.

**5.1c Current Owner**
- Opens directly into 5.2 for their own pet's breed.
- The Section 4.6 advisory never appears here. Poor bands always resolve to improvement-framed lines only.

**5.2 Breed detail**
- Screen title: the breed name (or "Your [breed]" for Current Owner).
- Per-factor breakdown, each written qualitatively, not scored: Space (Section 4.2/4.3), Safety (4.4), Noise fit (4.5, a single whole-home line rather than per room).
...
- **Animal avatar** (P4.3): moves through the space, reflecting the current room's band from Section 4.1. Example speech bubbles: dog with low floor space, "I need space to run"; cat with low vertical space, "I need space to jump up"; after an improvement and a rescan, "I can run now, yay!"
- If the Section 4.6 advisory is active, the avatar stays visibly concerned throughout this entire pass.

**6.3 Outside-3D summary report** (P4.4)
- Reachable any time via a button, or shown automatically at the end of the walkthrough.
- A plain list, grouped by room, on the cramped-to-spacious / hazardous-to-safe spectrum, plus one whole-home Noise Fit line at the top (Section 4.5), separate from the per-room lines. If the Section 4.6 advisory is active, it's the first line.
- Button: "Add pet-friendly items" leads to Stage 7, or "Back to dashboard."

### Stage 7: Pet-Proofing / Add Items

**7.1 Item selection**
- Two entry modes: **Select from inventory** (a catalogue filtered by species: bed, water bowl, litter box, scratching post/cat tree, crate, etc.), or **Digitally add** (place a generic placeholder for anything not in the catalogue).
...
- Bed/resting spot: checks against crowded/high-traffic zones.
- Vertical items (cat tree, shelving), cat personas only: checks against the vertical-space gap flagged in Stage 6.
...
- Re-runs the Section 4 evaluation, then returns to Stage 6 (or straight to the 6.3 summary) with updated results: resolved items drop off the highlight list, the avatar's mood and movement update per the new band, report lines rewrite (per the worked examples in 4.8-4.10). Noise Fit does not change on a rescan, only breed or neighbour-proximity changes touch it.
