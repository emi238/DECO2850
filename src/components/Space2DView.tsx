// The 2D room map for a space: a continuous panorama with severity-coloured,
// numbered hazard pins, a summary/verdict banner, tap-to-open finding cards, and
// a collapsible "all findings" list. Shared by the onboarding Results screen and
// the Home hub.
...
import type { Assessment, Frame, Hazard } from '../types';
        ...
        <GlassCard intensity={50} style={styles.banner}>
          ...
          <View style={styles.metaRow}>
            ...
            <Pressable onPress={() => setListOpen(true)}>
              <Text style={styles.listLink}>All findings ({result.hazards.length}) ›</Text>
            </Pressable>
          </View>
          {!!fallbackReason && <Text style={styles.fallback} numberOfLines={3}>{fallbackReason}</Text>}
        </GlassCard>
      </View>

      {/* ---- Panorama map with pins ---- */}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.map}>
        <View style={[styles.pano, { width: ADVANCE * Math.max(0, frames.length - 1) + FW, height: FH }]}>
          {frames.map((f, i) => (
...
const styles = StyleSheet.create({
  ...
  confidence: { color: colors.textFaint, fontSize: font.tiny, flexShrink: 1 },
  listLink: { color: colors.accent, fontSize: font.small, fontWeight: '700' },
  ...
  selectedWrap: { position: 'absolute', left: spacing.lg, right: spacing.lg, bottom: spacing.lg },
